...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    if (list_is_in_by_size(freelist, blocksize)) {
        block_t* curr_block = list_remove_from_front(freelist);
    }
    else {
    ...
    curr_block.pid = pid;
    // #4
    int original_block = curr_block.end;
    curr_block.end = curr_block->start + blocksize - 1;
    // #5
    list_add_ascending_by_address(curr_block);
    // #6 
    if (curr_block.end - curr_block.start < blocksize) {
      block_t* fragment = (block_t*)malloc(sizeof(block_t));
      fragment->pid = 0;
      fragment->start = curr_block.end + 1;
      fragment->end = original_block;
      switch (policy) {
        case 1:
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    if (list_is_in_by_pid(alloclist, pid)) // 1
      //2
      block_t* curr_block = list_remove_at_index(alloclist, list_get_index_of_by_Pid(alloclist, pid));
    else {
    ...
    switch (policy) {
      case 1:
        list_add_to_back(freelist, curr_blk);
        break;
      case 2: 
        list_add_ascending_by_blocksize(freelist, curr_blk);
        break;
      case 3: 
        list_add_descending_by_blocksize(freelist, curr_blk);
        break;
    }
}
